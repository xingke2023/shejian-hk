...
import { ChevronLeft, ChevronDown, Search, Calendar, X, Receipt } from "lucide-react";
...
export function PaymentStats() {
  ...
  const users = ["all", ...Array.from(new Set(allRecords.map((r) => r.user)))];
  const projects = [
    "all",
    ...Array.from(new Set(allRecords.map((r) => r.project))),
  ];

  const filteredRecords = allRecords.filter((record) => {
    const userMatch = filterUser === "all" || record.user === filterUser;
    const projectMatch =
      filterProject === "all" || record.project === filterProject;
    ...
    <div className="min-h-screen bg-gradient-to-b from-primary/5 to-white pb-20">
          ...
          <div className="flex-1">
            ...
            <div className="relative">
              <select
                ...
                className="w-full px-3 py-2 rounded-lg bg-input-background border border-border text-sm appearance-none"
              >
      ...
      <div className="px-6 py-4">
        <div className="bg-gradient-to-br from-primary to-accent text-white rounded-2xl p-6">
          <p className="text-sm opacity-90 mb-1">筛选结果总支付</p>
          ...
          <div className="flex items-center gap-4 mt-3 text-xs opacity-75">
            ...
            {filterUser !== "all" && <span>• {filterUser}</span>}
            {filterProject !== "all" && <span>• {filterProject}</span>}
          </div>
        </div>
      </div>
        ...
        <div className="space-y-3">
          {filteredRecords.length === 0 ? (
            <div className="bg-white rounded-xl p-8 text-center">
              ...
              <p className="text-muted-foreground">暂无符合条件的记录</p>
            </div>
          ) : (
            ...
            <div className="px-6 py-4 space-y-4">
              ...
              <div className="bg-muted/30 rounded-xl p-4 space-y-3">
                <div className="flex items-start justify-between py-2 border-b border-border/50">
                  ...
                  <span className="text-sm text-right">{selectedRecord.user}</span>
                </div>
                <div className="flex items-start justify-between py-2 border-b border-border/50">
